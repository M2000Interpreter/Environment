...
Dim monitor As Long, titl0$, titl$, once As Boolean
If once Then Exit Sub

once = True
If a = 13 Then
If pagio$ = "GREEK" Then
titl$ = "F1 - ανάλογα με την επιλεγμένη θέαση" + vbCrLf
titl$ = titl$ + "F1 Shift ή Control επιλέγει αυτή τη φόρμα" + vbCrLf
titl$ = titl$ + "F2 - Μετάβαση στο στοιχείο Εισαγωγή" + vbCrLf
titl$ = titl$ + "F3 - Αλλαγή θέασης (τρεις επιλογές)" + vbCrLf
titl$ = titl$ + "F4 - Θέαση στη Φόρμα Βοήθειας" + vbCrLf
titl$ = titl$ + "F5 - Επόμενο Βήμα" + vbCrLf
titl$ = titl$ + "* Με το Enter στο F5 δίνουμε πολλαπλά F5" + vbCrLf
titl$ = titl$ + "F6 - Στο Λάθος, ίσως το ξεπεράσουμε με το F5" + vbCrLf
titl$ = titl$ + "F7 - Αργή Ροή, τερματισμός στο λάθος (shift F1 για έλεγχο λάθους)" + vbCrLf
titl$ = titl$ + "F8 - Τερματισμός Εκτέλεσης" + vbCrLf
titl$ = titl$ + "f9 - Βήμα στο ίδιο επίπεδο" + vbCrLf
titl$ = titl$ + "F10 - Βήμα χωρίς έλεγχο επιπέδου" + vbCrLf
titl$ = titl$ + "F11 - Επαναφορά του ελέχου επιπέδου" + vbCrLf
titl$ = titl$ + "F12 - Λίστα Τμημάτων στη Φόρμα Βοήθειας" + vbCrLf
titl$ = titl$ + vbCrLf
titl$ = titl$ + "Κλίκ στο όνομα τμήματος δίνει το κώδικα στη Φόρμα Βοήθειας" + vbCrLf
titl$ = titl$ + "Κλίκ στην εντολή δίνει τη βοήθεια στη Φόρμα Βοήθειας" + vbCrLf
titl$ = titl$ + "Κλίκ στις παραμέτρους αλλάζει την Θέαση (ίδιο με το F4" + vbCrLf
titl$ = titl$ + vbCrLf
titl$ = titl$ + "Στην Εισαγωγή με αρχικό σύμβολο ? βάζουμε τύπους που θελουμε να εμφανίζονται σε κάθε εκτέλεση γραμμής" + vbCrLf
titl$ = titl$ + "Στην Εισαγωγή με αρχικό σύμβολο > δίνουμε άμεσα εντολές σαν να εκτελούνται στο κώδικα" + vbCrLf
titl$ = titl$ + "Στην Εισαγωγή όταν σβήνουμε όλα τα στοιχεία με Backspace αλλάζουμε το σύμβολο εισαγωγής" + vbCrLf

Else
titl$ = "F1 - depend on view" + vbCrLf
titl$ = titl$ + "F1 Shift or Control open this form" + vbCrLf
titl$ = titl$ + "F2 - select textbox" + vbCrLf
titl$ = titl$ + "F3 - change view (3 modes)" + vbCrLf
titl$ = titl$ + "F4 - view on help form" + vbCrLf
titl$ = titl$ + "F5 - Next Step" + vbCrLf
titl$ = titl$ + "* Press Enter on F5 for auto repeat" + vbCrLf
titl$ = titl$ + "F6 - Τo Error, may bypass error with F5" + vbCrLf
titl$ = titl$ + "F7 - Slow Flow, Stop Execution on error (Shift + F1 to check error)" + vbCrLf
titl$ = titl$ + "F8 - Stop Execution" + vbCrLf
titl$ = titl$ + "f9 - Step Over" + vbCrLf
titl$ = titl$ + "F10 - Step In" + vbCrLf
titl$ = titl$ + "F11 - Step Out" + vbCrLf
titl$ = titl$ + "F12 - List of Modules on help form" + vbCrLf
titl$ = titl$ + vbCrLf
titl$ = titl$ + "Click on module name to view code on Help Form" + vbCrLf
titl$ = titl$ + "Click on command name to view help on Help Form" + vbCrLf
titl$ = titl$ + "Click on parameters to change view (same as F4" + vbCrLf
titl$ = titl$ + vbCrLf
titl$ = titl$ + "On TextBox and prompt symbol ? place a list of expressios for view for each execution" + vbCrLf
titl$ = titl$ + "On TextBox and prompt symbol > execute statments inline" + vbCrLf
titl$ = titl$ + "On TextBox clear the line and press BackSpace to change prompt" + vbCrLf

End If

   If pagio$ = "GREEK" Then titl0$ = "Φόρμα Ελέγχου Πλήκτρα Λειτουργιών" Else titl0$ = "Control Form Function Keys"
        GoSub FindWindow
sHelp titl0$, titl$, (ScrInfo(monitor).Width - 1) * 3 / 5, (ScrInfo(monitor).Height - 1) * 4 / 7
        If Not Form4.Visible Then vHelp True
        Form4.label1.SelStartSilent = testpad.SelStart
        vHelp
ElseIf a = 12 Then
    interpret Basestack1, "help f12"
ElseIf a = 2 Then
gList0.SetFocus
ElseIf a = 3 Then
...
ElseIf a = 11 Then
' step out
If StepIn And Len(steptothat) > 0 Then
stoponerror = False
StepOver = True
StepIn = False
GoTo again9
...
If StepOver And steptothat = GetName(here$) Then
GoTo again5
End If
End If
If here$ <> vbNullString Then
...
StepIn = False
stephere$ = here$
steptothat = GetName(here$)
again9:
If pagio$ = "GREEK" Then
gList4.list(1) = "Επόμενο Μέσα F10"
Else
gList4.list(1) = "Step In F10"
End If
...
STbyST = True
If pagio$ = "GREEK" Then
gList4.list(1) = "Επόμενο F9"
Else
gList4.list(1) = "Step Over F9"
End If
...
gList4.ListIndex = 2
gList4_Selected2 2
ElseIf switchview = 1 And a = 1 Then
StepOver = False
StepIn = False
stoponerror = False
...
ElseIf StepIn Then
gList4.additemFast "Επόμενο F9"
ElseIf StepOver Then
gList4.additemFast "Επόμενο Μέσα F10"
...
ElseIf StepIn Then
gList4.additemFast "Step Over F9"
ElseIf StepOver Then
gList4.additemFast "Step In F10"
Else
...
Private Sub gList3_CtrlPlusF1(index As Integer)
generalFkey 13
End Sub
...
If pagio$ = "GREEK" Then
    Select Case that
    Case 0
        testpad.Title = "Σωρός & Μεταβλητές | F3: Θέαση | F4: Αντιγραφή στη βοήθεια"
        testpad.ReplaceTitle = " "
    Case 1
        
        testpad.Title = "Θέαση Κώδικα | F1: Καθαρό | F4: Αντιγραφή στη βοήθεια"
        testpad.ReplaceTitle = " "
    Case 2
        testpad.Title = "Καταγραφή Λαθών | F1: Καθαρή Λίστα | F4: Αντιγραφή στη βοήθεια"
        testpad.ReplaceTitle = " "
    End Select
Else
    Select Case that
    Case 0
        testpad.Title = "Stack & Variables | F3: Change View | F4: Copy to help form"
        testpad.ReplaceTitle = " "
    Case 1
        
        testpad.Title = "Code View |F1: Reset Form | F4: Copy to help form"
        testpad.ReplaceTitle = " "
    Case 2
        testpad.Title = "Error Log | F1: Clear Error | F4: Copy to help form"
        testpad.ReplaceTitle = " "
    End Select
End If
End Sub
