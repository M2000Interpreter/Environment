                        ...
                        If ((KeyAscii = 9 And UseTab) Or (KeyAscii > 31) And SELECTEDITEM > 0) Then
                            ...
                            If EditFlagSpecial And ((KeyAscii > 32 And KeyAscii <> 127) Or (KeyAscii = 9 And UseTab)) Then
                                ...
                                If KeyAscii = 44 And Len(kk$) = 2 Then
                                    ...
                                    RaiseEvent PureListOff
                                ElseIf AscW(kk$) > -10241 And AscW(kk$) < -9216 Then
                                    RaiseEvent SetExpandSS(mSelstart + 2)
                                    SelStartEventAlways = SelStart + 2
                                    RaiseEvent PureListOn
                                    pair$ = list(SELECTEDITEM - 1)
...
Pos = SelStart
If Pos <> 0 Then
Dim mypos As Long, ogt As String, this$, posWWW As Long
posWWW = -1
If Pos <= 0 Then Pos = 1
ePos = Pos
...
If Me.WWWLink Then
    If Pos > 3 Then
        ...
        If Mid$(mline$, Pos - 2, 3) = "://" Then
            posWWW = Pos - 3
cont1:
            Do While posWWW > 0
                If InStr(1, WordCharLeft, Mid$(mline$, posWWW, 1)) Then Exit Do
                posWWW = posWWW - 1
            Loop
            If posWWW = Pos - 3 Then
                posWWW = -1
            Else
                Pos = posWWW
            End If
        Else
        If Mid$(mline$, Pos, 1) = "/" Then
            posWWW = Pos
            Do While posWWW > 0
                one$ = Mid$(mline$, ePos, 1)
                If InStr("-._~$+!*%()/\=?&:#;,", one$) = 0 Then
                    If InStr(1, WordCharRight, one$) Then Exit Do
                End If
                posWWW = posWWW - 1
            Loop
            posWWW = InStr(posWWW + 1, mline$, "://")
            If posWWW = 0 Then
                posWWW = -1
            Else
                posWWW = posWWW - 1
...
If (ePos - Pos - 1) > 0 Then
    If Me.WWWLink Then
        If posWWW = -1 And Mid$(mline$, ePos, 3) = "://" Then
            posWWW = Pos
            GoTo cont2
        ElseIf posWWW = -1 And InStr(Mid$(mline$, Pos + 1, ePos - 1), "://") > 0 Then
            posWWW = Pos
            GoTo cont2
        End If
    End If
...
End Sub
Public Function VirtualKey(ascii As Integer) As Long
    VirtualKey = MapVirtualKey(ascii, 2)
End Function
