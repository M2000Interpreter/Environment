...
Public Function CreateBi(v As Variant) As BigInteger
    If MyIsNumeric(v) Then
        Set CreateBi = CreateBigInteger(format(Int(v), "0"))
    
    ElseIf MemInt(VarPtr(v)) = vbString Then
        Dim s$
        SwapString2Variant s$, v
        Set CreateBi = CreateBigInteger(s$)
    End If
