...
Private acckeys As FastCollection
Private m_opengl As Boolean
...
If mIndex >= 0 Then
    Callback mMyName$ + ".OpenInfo(" + str(mIndex) + "," + str(gl) + ")"
Else
    Callback mMyName$ + ".OpenInfo(" + str(gl) + ")"
End If
...
var(0) = 0&
If mIndex > -1 Then
If Not Quit Then CallEventFromGuiNow Me, myEvent, mMyName$ + ".Unload(" + str(mIndex) + ")", var()
Else
...
End If
ElseIf Y + that.Height > ScrInfo(hmonitor).Height + ScrInfo(hmonitor).top Then
If Y - that.Height - y1 < ScrInfo(hmonitor).top Then
that.move X, ScrInfo(hmonitor).top
Else
...
'
  If mIndex >= 0 Then
   Callback mMyName$ + ".InfoChecked(" + str(mIndex) + "," + str(glistN.ListIndex) + ")"
   Else
      Callback mMyName$ + ".InfoChecked(" + str(glistN.ListIndex) + ")"
      End If
End Sub
...
Else
glistN.MenuItem item, checked, radiobutton, firstate, str(IdD)
End If
...
Else
If IsNumeric(IdD) Then IdD = Int(IdD)
...
Else
glistN.MenuItem item, True, True, False, str(IdD)
End If
...
If RHS > 0 Then
    MenuItem "About", True
    MenuItem ""
    MenuItem "Minimize", drawminimized And Not IsWine And Not IamPopUp
...
Else
    MenuItem "”˜ÂÙÈÍ‹", True
    MenuItem ""
    MenuItem "¡¸ÍÒı¯Á", drawminimized And Not IsWine And Not IamPopUp
