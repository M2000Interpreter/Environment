...
Public Function SaveImageToPngByteArray(Img As Long) As Object
    
    Dim MyBlock As MemBlock
    ...
    If GdiPlusExec(GdipSaveImageToStream(Img, ObjPtr(Strm), mPngID(0), ByVal 0&)) = ok Then
        If GdiPlusExec(GdipSaveImageToStream(Img, ObjPtr(Strm), mPngID(0), ByVal 0&)) = ok Then
            IStream_Size ObjPtr(Strm), SizeLL(0)
            If SizeLL(0) > 0 Then
                Set MyBlock = New MemBlock
                MyBlock.Construct SizeLL(0)
            Else
                Exit Function
            End If
            IStream_Reset ObjPtr(Strm)
            IStream_Read ObjPtr(Strm), ByVal MyBlock.GetPtr(0), SizeLL(0)
            Set SaveImageToPngByteArray = MyBlock
        End If
    End If
End Function
...
Public Function LoadPictureGDIPlus(picFile As String, Optional Width As Long = -1, Optional Height As Long = -1, Optional ByVal BackColor As Long = vbWhite) As IPicture
    ...
    If GdipLoadImageFromFile(StrPtr(picFile), Img) <> 0 Then
        ...
        MyEr "GDI+ - can't load picture", "GDI+ - δεν μπορώ να φορτώσω την εικόνα"
        Exit Function
    End If
    ...
    Bitmap = True
     Else
        ...
        If orWidth = 0 Or orHeight = 0 Then Exit Sub
          ...
          If bleft < 0 Then
          'bWidth = bWidth - bleft * 0: Width = Width + 4 * dv15
         ' orWidth = Width
          End If
          If btop < 0 Then
          
          'bHeight = bHeight - btop * 0: Height = Height + 4 * dv15
          ' orHeight = Height
          End If
...
zoomfactor = zoomfactor / 100#
 ...
 Const ColorAdjustTypeText = 4&
    ...
    'End If
      ...
      If (.MAXXGRAPH / dv15 - bWidth) > 0 Then ax = ax + (.MAXXGRAPH / dv15 - bWidth) / 2
        If (.MAXYGRAPH / dv15 - bHeight) > 0 Then ay = ay + (.MAXYGRAPH / dv15 - bHeight) / 2
        'If (.MAXXGRAPH / dv15 - bWidth) > 0 Then ax = ax + (.MAXXGRAPH / dv15 - bWidth * Size)
        'If (.MAXYGRAPH / dv15 - bHeight) > 0 Then ay = ay + (.MAXYGRAPH / dv15 - bHeight * SizeY)
       If ax < 0 Then ax = 0
       If ay < 0 Then ay = 0
       If ax > ay Then ay = ax Else ax = ay
    ...
    GdipRotateWorldTransform graphics, Angle!, 0&
    With players(GetCode(Scr))
     If Scr Is Form1.PrinterDocument1 Then
     GdipTranslateWorldTransform graphics, Scr.ScaleX(.XGRAPH, 0, 3), Scr.ScaleY(.YGRAPH, 0, 3), 1
     Else
...
End Function
Public Function PngImageFromBuffer(ResData() As Byte) As Object
    Dim Stream As IUnknown
    Dim Img As Long
    Call CreateStreamOnHGlobal(VarPtr(ResData(0)), False, Stream)
    If Not (Stream Is Nothing) Then
        If GdiPlusExec(GdipLoadImageFromStream(Stream, Img)) = ok Then
            Set PngImageFromBuffer = SaveImageToPngByteArray(Img)
            Call GdiPlusExec(GdipDisposeImage(Img))
        End If
    End If
End Function
