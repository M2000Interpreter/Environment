...
Dim i As Long, v$, ss1$, nomore As Boolean, yesvol As Boolean, probe2play As Integer
ss1$ = ss$ & "_"
note2play = 0
i = 1
...
ElseIf Len(ss$) > 0 Then
    ...
    End If
End If
''Debug.Print v$ + ".", probe2play, Len(ss$), "?" + ss$ + "?"
If probe2play = 25 Then
    note2play = 24
    If Mid$(ss$, i + 1, 1) = "@" Then
        i = i + 1
        If InStr("123456", Mid$(ss1$, i + 1, 1)) > 0 Then
            subbeat = val(Mid$(ss1$, i + 1, 1))
            i = i + 1
        End If
    End If
    If UCase$(Mid$(ss1$, i + 1, 1)) = "V" Then
        i = i + 1
        v$ = vbNullString
        Do While InStr("1234567890", Mid$(ss1$, i + 1, 1)) > 0 And (Mid$(ss1$, i + 1, 1) <> "")
            v$ = v$ & Mid$(ss1$, i + 1, 1)
            i = i + 1
        Loop
        volume2play = val("0" & v$)
...
Else
    ...
    If probe2play <> 0 Then
        ' look for number
        If Mid$(ss1$, i + 1, 1) <> "" Then
            If InStr("0123456789", Mid$(ss1$, i + 1, 1)) > 0 Then
                octave2play = val(Mid$(ss1$, i + 1, 1))
                i = i + 1
            End If
            If Mid$(ss1$, i + 1, 1) = "@" Then
                i = i + 1
            ...
            If UCase$(Mid$(ss1$, i + 1, 1)) = "V" Then
                i = i + 1
                v$ = vbNullString
                Do While InStr("1234567890", Mid$(ss1$, i + 1, 1)) > 0 And (Mid$(ss1$, i, 1) <> "")
                    v$ = v$ & Mid$(ss1$, i + 1, 1)
                    i = i + 1
                Loop
                volume2play = val("0" & v$)
...
th:
    ...
    If note2play = 0 Then ss$ = "": Exit Function
'    Debug.Print i, Left$(ss$, i), ">>>", Mid$(ss$, i + 1)
    ss$ = Mid$(ss$, i + 1)
...
Case "INSERT", "INSTR(", "INT(", "INTEGER", "INTEGER[", "INTERFACE", "INTERVAL", "INTERNET", "INTERNET$", "INVENTORY", "IS", "ISLET", "ISNUM", "ISWINE", "ITALIC"
...
Case "LOWORD(", "LOWWORD(", "LTRIM$(", "MAIN.TASK", "MAGNITUDE(", "MAP(", "MARK", "MASTER", "MAT(", "MATCH(", "MAX(", "MAX.DATA$("
...
Case "PLAYER", "PLAYSCORE", "PLAYNOTE(", "PLAYNOW(", "PLAYVALUE(", "PLAYVOLUME("
...
Case "REMOVE", "REPEAT", "REPLACE$(", "REPORT", "REPORTLINES", "RESTART", "RESTORE", "RETRIEVE", "RETURN", "REV(", "REVISION"
...
Case "TO", "TODAY", "TONE", "TOP", "TRIM$(", "TRUE", "TRY", "TUNE", "TUPLE", "TWIPSX"
...
Case "WITH", "WITHEVENTS", "WORDS", "WRITABLE(", "WRITE", "WRITER", "X.TWIPS", "XOR", "Y.TWIPS", "аццкийа", "адеиас", "аниысг", "апеияо", "апой.ая$(", "апой.де$("
Case "адеиасе", "аежап(", "агл(", "ахя(", "ай(", "айеяаио.дуадийо(", "айеяаио", "айеяаиос", "айеяаиос[", "акт", "акгхес", "акгхгс", "аккацг", "аккацг$("
...
Case "амехесе", "амоицла(", "амоицла.аявеиоу", "амоицла.еийомас", "амоине", "амтецяаье", "амтицяаье", "амтийеилемо(", "амы", "аниа", "аниа(", "аниа$", "аниа(", "апая", "апаяихлгсг", "апедысе"
Case "апко", "апкос", "апкос[", "апо", "апохгйеусг.ыс", "апой$(", "апойопг", "апойкеистийа", "апок(", "аяца", "аяихло", "аяихлос", "аяихлос[", "аяихлос.паяацяажоу("
...
Case "кабг.амакоцийо.в(", "кабг.йатеухумсг(", "кахос", "кахос$", "кахос.таимиас$", "кахым", "калда", "калда(", "калда$", "калда$(", "катимийа", "кенеис", "киста", "коц("
Case "коцийо", "коцийос", "коцистийо", "коцистийос", "коцистийос[", "коцос", "коцос$(", "кс$", "кж(", "лафи(", "лайяу", "лайяус", "лайяус[", "ле", "ле.амтийеилемо", "лец(", "лец$(", "лецако("
Case "лецако.сеияас$(", "лецако.сеияас(", "лецакосайеяаиос", "лецакосайеяаиос[", "лецакосайеяаиос(", "лецецомота", "лецехос", "лецехос.сыяоу", "лецехос.у(", "лецехос.в(", "лецехос.ь(", "леходос", "лекос$(", "лекоус.тупос$("
...
Case "лицадийос", "лицадийос[", "лий(", "лий$(", "лийяо(", "лийяо.сеияас$(", "лийяо.сеияас(", "лийяос.йатакоцос$(", "лмглг", "ломадиаио", "ломадиаиос", "ломадийо", "лояжг$(", "лоусийг", "лоусийг.летягтгс", "лпип"
...
Case "паине", "паифеианиа(", "паифеиемтасг(", "паифеимота(", "паифеитыяа(", "пай(", "пай$(", "памта"
...
Case "выяисла", "ьеудес", "ьеудгс", "ьгжио", "ьгжио[", "ь.сглеиа", "ыс"
Case Else
ismine = False
End Select
End Function
...
103, "IMAGE(", 103, "BUFFER(", 104, "диаяхяысг(", 104, "BINARY.NOT(", 105, "дуадийо.ови(", 105, "POINTER(", 108, "деийтгс(", 108, "BINARY.ADD(", 109, "дуадийо.пяосхесг(", 109, "дуадийо.пяо(", 109, "HSL(", 110, "вйж(", 110, "PLAYER(", 111, "паийтгс(", 111, "GETOBJECT(", 112, "амтийеилемо(", 112, "VARPTR(", 113, "диеухл(", 113, "BIGINTEGER(", 114, "лецакосайеяаиос(", 114 _
...
"RATN(", 125, "атон.еж(", 125, "PLAYVALUE(", 126, "паифеианиа(", 126, "PLAYVOLUME(", 127, "паифеиемтасг(", 127, "PLAYNOTE(", 128, "паифеимота(", 128, "PLAYNOW(", 129, "паифеитыяа(", 129)
...
, "еисацыцг", "ейдосг", "ейтекесг", "ейтупысг", "ейтупытгс", "екецвос", "еккгмийа", "емхесг", "емтасг", "емы", "емысе", "енацыцг", "енодос", "епамакабе", "епамажояа", "епамекабе", "епекене", "епекене.амтийеилемо", "епекене.цяаллатосеияа", "епекене.ояцамо", "епекене.вяыла", "епицяажг", "епийаияо", "епикене", "епикене.амтийеилемо", "епикене.цяаллатосеияа", "епикене.ояцамо", "епикене.вяыла", "епикоцес", "епикоцг", "епикоцгс" _
...
Case "поиотгта.ейтупысгс", "PRINTER.QUALITY"
