...
Public Function FindDISPIDExt(pobjTarget As Object, ByVal pstrProcName As Variant) As Long
    ...
    Dim riid        As IDispatch.IID
    Dim pp      As Long
    Dim E1 As ExtControl, usehandler As mHandler
    FindDISPIDExt = -1
    If pobjTarget Is Nothing Then Exit Function
    ...
    ElseIf TypeOf pobjTarget Is mHandler Then
        Set usehandler = pobjTarget
        Set IDsp = usehandler.objref
        Set pobjTarget = IDsp '
    Else
        Set IDsp = pobjTarget
    End If
...
Public Function CallByNameLetValue _
    (pobjTarget As Object, _
    ByVal pstrProcName As Variant, _
    ByVal CallType As cbnCallTypes, _
     pArgs(), pargs2() As String, _
     items As Long, fixnamearg As Long, shiftarray As Boolean) As Boolean
'Debug.Print Typename(pobjTarget), pstrProcName, CallType
    ...
    If fixnamearg = 0 Then
        ...
        End If
    Else
        ReDim myptr(0 To fixnamearg + 1)
        myptr(0) = StrPtr(pstrProcName)
        For lngLoop = fixnamearg To 0 Step -1
...
passhere:
        If items > 0 Or fixnamearg > 0 Then
                ...
                ElseIf Not MyIsNumericPointer(pArgs(lngLoop)) Then
                    ...
                    ElseIf TypeOf pArgs(lngLoop) Is mArray Then
                        ...
                        If mmm.IsByValue Then
                            varArr(fixnamearg + items - 1 - lngLoop) = mmm.VariantArray()
                        Else
                            mmm.ExportAndDestroy VarPtr(varArr2(fixnamearg + items - 1 - lngLoop))
            ...
            With params
                ...
                If CallType = VbLet Or CallType = VbSet Or fixnamearg > 0 Then
                    If fixnamearg = 0 Then ReDim varDISPID(0 To 0)
                    varDISPID(0) = DISPID_PROPERTYPUT
                    .cNamedArgs = fixnamearg + 1
        ...
        If lngRet < 0 Then
            If Excep.sCode = 0 Then
            FixError Hex$(lngRet)
            
            Else
            FixError Hex$(Excep.sCode)
            End If
            GoTo ExitHere
            'If lngRet = DISP_E_EXCEPTION Then
            '    ' CallByName pobjTarget, pstrProcName, VbMethod
            '    If Excep.StrPtrDescription = 0 Then
            '        If ComErrorDesc.ExistKey(CVar(Excep.sCode)) Then
            '            MyEr "COM error 0x" + Hex$(Excep.sCode) + " " + ComErrorDesc.Value, "COM λάθος 0x" + Hex$(Excep.sCode) + " " + ComErrorDesc.Value
            '        Else
            '            MyEr "COM error 0x" + Hex$(Excep.sCode), "COM λάθος 0x" + Hex$(Excep.sCode)
            '        End If
            '    Else
        ...
        End If
    Else
        MyEr "not supported method/property: " + pstrProcName, "Δεν υποστηρίζεται η μέθοδος ή ιδιότητα: " + pstrProcName
...
there:
    Err.Clear
    CallByNameLetValue = Err.Number = 0
    Exit Function
'ExitHere:
'    CallByNameLetValue = Err.Number = 0
'    Err.Clear
...
'Debug.Print Typename(pobjTarget), pstrProcName, CallType
    ...
    ' pargs2() the names of arguments
    ' fixnamearg = the number of named arguments
    Dim usehandler As mHandler, myEnum As Enumeration
    Dim myform As Form
    Dim IDsp        As IDispatch.IDispatchM2000
    Dim riid        As IDispatch.IID
    Dim params      As IDispatch.DISPPARAMS
    ...
    Dim dispid      As Long
    Dim lngArgErr   As Long
    Dim VarRet      As Variant, ud As Variant
    Dim varArr()    As Variant
    Dim varDISPID() As Long
    Dim lngRet      As Long
    Dim lngLoop     As Long
    ...
    If Not UseExtControl Then
        ...
        Else
        If TypeOf pobjTarget Is Control Then
        useExtender = True
        End If
        End If
...
passhere:
        If items > 0 Or fixnamearg > 0 Then
            ...
            For lngLoop = 0 To items - 1 + fixnamearg
                If myIsNull(pArgs(lngLoop)) Then
                    SwapVariant varArr(fixnamearg + items - 1 - lngLoop), pArgs(lngLoop)
                ElseIf MemInt(VarPtr(pArgs(lngLoop))) = 36 Then
                    If TypeOf pArgs(lngLoop) Is ExtArray Then
                        ud = pArgs(lngLoop)
                        nMath2.DumpVBArray VarPtr(ud)
                        SwapVariant varArr(fixnamearg + items - 1 - lngLoop), ud
                    Else
                        SwapVariant varArr(fixnamearg + items - 1 - lngLoop), pArgs(lngLoop)
                    End If
                ElseIf Not MyIsNumericPointer(pArgs(lngLoop)) Then
                    ...
                    ElseIf TypeOf pArgs(lngLoop) Is MemBlock Then
                        varArr2(fixnamearg + items - 1 - lngLoop) = pArgs(lngLoop).ExportToByte
                        VarByRef VarPtr(varArr(fixnamearg + items - 1 - lngLoop)), varArr2(fixnamearg + items - 1 - lngLoop)
                    ElseIf TypeOf pArgs(lngLoop) Is Enumeration Then
                        Set myEnum = pArgs(lngLoop)
                        varArr2(fixnamearg + items - 1 - lngLoop) = myEnum.Value
                        VarByRef VarPtr(varArr(fixnamearg + items - 1 - lngLoop)), varArr2(fixnamearg + items - 1 - lngLoop)
        ...
        ' Invoke method/property
        If useExtender Then
        If dispid = &H80011001 Then
        GoTo jumpe1
        End If
        End If
        If LastErNum = 0 Then
            lngRet = IDsp.Invoke(dispid, riid, 0, CallType, params, VarRet, Excep, lngArgErr)
            If lngRet = -2147352573 And useExtender Then
            GoTo jumpe1
            ElseIf lngRet = -2147352562 Then
                lngRet = IDsp.Invoke(dispid, riid, 0, 2, params, VarRet, Excep, lngArgErr)
            End If
        End If
        If LastErNum <> 0 Then GoTo ExitHere
        If lngRet < 0 Then
            ...
            If (TypeOf IDsp Is GuiM2000) And Not lngRet = DISP_E_EXCEPTION Then
JUMPHERE:
                On Error GoTo ExitHere
                lngRet = 0
...
jumpe1:
                    If fixnamearg > 0 Then MyEr "no named arguments for this call", "όχι ορίσματα με ονόματα για αυτή τη κλήση"
                    Select Case items
                    Case 0
                        CallByName pobjTarget, pstrProcName, VbMethod
                    Case 1
        ...
        For lngLoop = 0 To items - 1 + fixnamearg
           ...
           If Not MyIsNumericPointer(pArgs(lngLoop)) Then
                    If Not MyIsNumericPointer(varArr(where)) Then
                        ...
                        ' do nothing
                        VarByRefClean VarPtr(varArr(where))
                     '   ElseIf VarType(varArr(where)) = 8204 Then
                     '       VarByRefClean VarPtr(varArr(where))
                     '       Set mmm = pArgs(lngLoop)
                     '       mmm.FixArray
                     '       Set mmm = Nothing
...
contNext:
    ...
    If IsObject(VarRet) Then
            Set robj = VarRet
         VarRet = 0&
    ElseIf (MemInt(VarPtr(VarRet)) And &H2000) <> 0 Then
        Err.Clear
        nMath2.CompactVBArray VarPtr(VarRet), VarPtr(CallByNameFixParamArray)
        Exit Function
    End If
On Error GoTo there
If TypeOf VarRet Is IUnknown Then
...
Public Function ReadOneParameter(pobjTarget As Object, dispid As Long, ERrR$, VarRet As Variant) As Boolean
    ...
    Set IDsp = Nothing
    If (MemInt(VarPtr(VarRet)) And &H2000) <> 0 Then
        Dim th
        nMath2.CompactVBArray VarPtr(VarRet), VarPtr(th)
        SwapVariant VarRet, th
    End If
...
Else
        If lngRet <> 0 Then
            If lngRet = DISP_E_EXCEPTION Then
             ERrR$ = str$(Excep.wCode)
             If Excep.wCode = 0 Then
             ERrR$ = Hex$(Excep.sCode)
             End If
            Else
              ERrR$ = str$(lngRet)
            End If
  ...
  End If
    ...
    If IsObject(VarRet) Then
        ...
        End If
    ElseIf (MemInt(VarPtr(VarRet)) And &H2000) <> 0 Then
        nMath2.CompactVBArray VarPtr(VarRet), VarPtr(ReadOneIndexParameter)
    Else
